...
async function profileViewsValue() {
  const apiUrl = process.env.PROFILE_VIEWS_API;
  if (apiUrl) {
    try {
      const response = await fetch(apiUrl, {
        headers: { "User-Agent": "github-code-solar-system" },
      });
      if (!response.ok) throw new Error(`${response.status} ${response.statusText}`);
      const data = await response.json();
      const value = Number(data.views);
      if (data.since && !CONFIG.profileViewsSince) CONFIG.profileViewsSince = data.since;
      if (Number.isFinite(value) && value >= 0) return Math.floor(value);
      throw new Error("response does not contain a valid views number");
    } catch (error) {
      console.warn(`Profile views unavailable: ${error.message}`);
    }
  }
...
async function main() {
  ...
  const views = demo ? 12482 : await profileViewsValue();
